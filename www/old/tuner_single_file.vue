<template>
  <f7-page>
    <f7-content-block inset>
      <div id="detector" v-bind:class=" pitch == 0 ? 'vague' : 'confident' ">
        <f7-grid>
          <f7-col width="100">
      ...
      <f7-button raised v-on:click="toggleLiveInput">{{ isPlaying ? "Stop" : "Start" }}</f7-button>
    </f7-content-block>
  </f7-page>
</template>
...
export default {
  methods: {
    ...
    permission_refused: function() {
      alert( 'Cannot access microphone without permission. Please grant app permission.' )
    },
    permission_error: function() {
      alert( 'Error getting permssion. Try setting app permissions, or reinstall app.' )
    },
